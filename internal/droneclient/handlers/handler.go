...
func (h *Handler) StartEnpointUdpClientHandle(httpHost, httpPort, dialect string, OutSystemID byte) {
	...
	if err != nil {
		h.log.Error(fmt.Sprintf("Endpoint UDP Client with %v couldn't start:", Address), zap.Error(err))
	}

	h.log.Info(fmt.Sprintf("Udp endpoint Client with id:%v and adress:%v starting with dialect:%v", OutSystemID, Address, dialect))
